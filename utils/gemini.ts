...
export const getActionItems = async (text: string, lang: Language): Promise<ActionItem[]> => {
    // This function can now take the full context from all sources
    try {
        const ai = getAIClient();
...
export const getKeyTopics = async (text: string, lang: Language): Promise<string[]> => {
    // This function can now take the full context from all sources
    try {
        const ai = getAIClient();
...
export const getProofreadAndStyledText = async (text: string, style: TextStyle, lang: Language): Promise<string> => {
...
export const getStyledText = async (text: string, style: TextStyleId, lang: Language): Promise<string> => {
    try {
        const ai = getAIClient();
        const styleName = t(`style${style.charAt(0).toUpperCase() + style.slice(1)}` as any, 'en');
...
export const getAgentResponse = async (
    ...
    try {
        ...
        const contentsWithContext = [...chatHistoryWithPrompt];
        
        // Find the last message, which is the current user prompt, and prepend the context to it.
        const lastMessageIndex = contentsWithContext.length - 1;
        if (lastMessageIndex >= 0 && contentsWithContext[lastMessageIndex].role === 'user' && context.trim()) {
            const originalPrompt = contentsWithContext[lastMessageIndex].parts[0].text;
            const contextHeader = `Based *only* on the following source documents, answer the user's query.\n\n--- SOURCES ---\n${context}\n--- END SOURCES ---\n\n`;
            contentsWithContext[lastMessageIndex].parts[0].text = contextHeader + `USER QUERY: "${originalPrompt}"`;
        }
...
export const extractEntities = async (text: string, lang: Language): Promise<Entity[]> => {
    // This function primarily works on the main transcription, not all sources.
    try {
        const ai = getAIClient();
...
export const getSourceGuide = async (content: string, lang: Language): Promise<SourceGuide> => {
    try {
        const ai = getAIClient();
...
export const getDiscussionForTopic = async (topic: string, context: string, lang: Language): Promise<string> => {
    try {
        const ai = getAIClient();
...
export const getEmotionAnalysis = async (text: string, lang: Language): Promise<string> => {
    try {
        const ai = getAIClient();
...
export const getTonalityAnalysis = async (text: string, lang: Language): Promise<string> => {
    try {
        const ai = getAIClient();
...
export const getBrainstormIdeas = async (text: string, lang: Language): Promise<string> => {
    try {
        const ai = getAIClient();
...
export const getGrammarCheck = async (text: string, lang: Language): Promise<string> => {
    try {
        const ai = getAIClient();
...
export const getPromptWizardResponse = async (history: AIChatMessage[], lang: Language): Promise<string> => {
    try {
        const ai = getAIClient();
...
export const getInsightsForText = async (text: string, lang: Language): Promise<Insight[]> => {
    try {
        const ai = getAIClient();
